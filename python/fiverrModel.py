...
class FiverrIntegrationModel:
    ...
    async def find_matches(self, project_details: Dict) -> List[Dict]:
        try:
            conn = await self.get_db_connection()
            cursor = conn.cursor(dictionary=True)

            # Get all sellers with their gigs
            cursor.execute("""
                SELECT u.*, g.* 
                FROM users u
            ...
            freelancers = [
                Freelancer(
                    ...
                    name=s['username'],
                    job_title=s['title'],
                    skills=s['description'].split(','),
                    experience=1,
                    rating=s['total_stars']/s['star_number'] if s['star_number'] > 0 else 0,
                    hourly_rate=s['price'],
                    profile_url=s['img'],
                    availability=True,
                    total_sales=s['star_number']
                ) for s in sellers
            ]
            ...
            project = Project(
                ...
                required_skills=self.skill_extractor.extract_skills(project_details['description']),
                budget_range=(project_details['min_budget'], project_details['max_budget']),
                complexity='medium'
            )
            ...
            # Get matches
            matches = self.matching_engine.get_top_matches(project)

            return [
                {
                    'id': match['freelancer'].id,
                    'username': match['freelancer'].username,
                    'description': match['freelancer'].job_title,
                    'skills': match['freelancer'].skills,
                    'price': match['freelancer'].hourly_rate,
                    'rating': match['freelancer'].rating,
                    'img': match['freelancer'].profile_url,
